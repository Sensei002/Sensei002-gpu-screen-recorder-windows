...
| `capture.h` | capture backend identity + auto-selection (WGC / DXGI) + the WGC and DXGI backends' C APIs and pure logic | engine `gsr_capture` vtable (kept unchanged) — this header is the *selection* layer + the backends' extern "C" surfaces | `gsr_platform_win32.c` (select/name); `gsr_capture_wgc.cpp` (WGC backend, C++/WinRT, extern "C" API) + `gsr_capture_wgc_helpers.c` (pure logic); `gsr_capture_dxgi.c` (DD backend, plain C) + `gsr_platform_capture_dxgi_available()` probe | 3 ✅ logic, 5 ✅ WGC backend + self-test, 6 ✅ DD backend + self-test (REAL capture on CI's Basic Display Adapter) |
| `egl_win32.h` | Windows ANGLE GL backend: shared D3D11 device accessor + D3D11-texture import (`EGL_ANGLE_d3d_texture_client_buffer`) | engine `gsr_egl` struct / `color_conversion.c` (kept unchanged — upstream GL pipeline runs on ANGLE, architecture §3.3 Option B); WGC backend `capture()` | `gsr_egl_win32.c` (ANGLE loader, surfaceless ES3 ctx on D3D11 + import helpers) + `_WIN32` seam in upstream `egl.c` | 5b ✅ render pipeline (CI: WARP) |
...
|---|---|
| `src/capture/*` (portal, xcomposite, ximage, kms, nvfbc, v4l2) | `gsr_capture` vtable unchanged; backends are `capture.h`-selected WGC (Phase 5, shipped: `gsr_capture_wgc.cpp`) / DXGI duplication (Phase 6, shipped: `gsr_capture_dxgi.c`) |
