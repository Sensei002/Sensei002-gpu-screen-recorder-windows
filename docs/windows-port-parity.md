...
This matrix is a living document; Phase 19 (parity testing) drives every row
...
| `-a` audio sources (`default_output`, `default_input`, `device:`, `app:`, `app-inverse:`, `\|`) | FULL | device/default FULL; `app:` PARTIAL | PLANNED | WASAPI sessions; feasibility Phase 10 |
| `-ac`/`-ab` (opus/aac; flac disabled upstream) | FULL | FULL | PLANNED | keep flac disabled for parity |
| `-k` codecs (h264/hevc/av1/vp8/vp9 + `_hdr`/`_10bit` + `_vulkan`) | FULL | h264/hevc/av1 via nvenc/amf/qsv/software; vp8/vp9 via software | PARTIAL | `_vulkan` variants NOT POSSIBLE unless Vulkan encode kept; hidden |
...
| `-low-power` | AMD (VAAPI) | AMF equivalent if feasible | PARTIAL | |
...
| AMF (AMD) | n/a (VAAPI) | h264/hevc/av1_amf | WINDOWS-SPECIFIC | |
| QSV (Intel) | n/a (VAAPI) | h264/hevc/av1_qsv | WINDOWS-SPECIFIC | |
| VAAPI | FULL | NOT POSSIBLE | NOT POSSIBLE | replaced by AMF/QSV |
...
| System tray | TODO upstream (in gtk frontend) | minimal tray icon (decision Phase 12) | WINDOWS-SPECIFIC | |
